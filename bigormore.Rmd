...
---
title: "Publish Big or Publish More - A Mathematical Framework for Deciding Whether to Bundle or Separate Scientific Findings"
author: Wanjun Gu
...
Each of these scientific findings has a random **value distribution** \( F_i \) which we interpret as the overall expected return to the researcher _and_ to the field. This can reflect impact, utility, visibility, contribution, readability, long-term scientific influence, etc.
...
\[
\text{Total utility if more than one finding is published is additive: }
V = \sum_{i=1}^k X_i.
\]
...
\[
REV_S = \sum_{i=1}^k \sup_{p \ge 0} \; p \cdot \Pr[X_i > p]
\]
...
\[
REV_S = \sum_{i=1}^k \sup_{p \ge 0} p \cdot (1 - F_i(p))
\]
...
* If the studies differ wildly in expected distribution, publishing separately is safer.
